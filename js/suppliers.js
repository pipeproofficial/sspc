...
window.editSupplier = async (id) => {
    ...
    if (document.getElementById('supplierVillage')) document.getElementById('supplierVillage').value = s.village || '';
    await setStateDistrictValues(
        document.getElementById('supplierState'),
        document.getElementById('supplierDistrict'),
        s.state || '',
    ...
    if (document.getElementById('supplierMandal')) document.getElementById('supplierMandal').value = s.mandal || '';
...
window.deleteSupplier = async (id) => {
    if (!confirm('Delete this supplier and all related supplier/material transactions?')) return;
    const user = JSON.parse(localStorage.getItem('user'));
    ...
    try {
        ...
        const [deletedBySupplierId, deletedPurchases] = await Promise.all([
        ...
        const deletedBySupplierName = supplierName
            ? await deleteQueryInBatches(root.collection('transactions').where('type', '==', 'SupplierPayment').where('supplier', '==', supplierName))
            : 0;
        let fallbackDeleted = 0;

        if (normalizedSupplierName) {
            const [allPurchasesSnap, allSupplierTxSnap] = await Promise.all([
        ...
        const totalRelated = deletedBySupplierId + deletedPurchases + deletedBySupplierName + fallbackDeleted;

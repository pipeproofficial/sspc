...
function bindEvents() {
    ...
    window.addEventListener('paymentsUpdated', async () => {
        if (window.location.hash === '#audit') {
            await loadAuditReport();
        }
    });
...
function isLinkedSupplierPayment(payment, supplierIds, supplierNames) {
    if (!payment) return false;
    const id = String(payment.supplierId || '').trim();
    if (id && supplierIds.has(id)) return true;
    const name = normalizeText(payment.supplier || '');
    return !!name && supplierNames.has(name);
}

function isLinkedPurchase(purchase, supplierNames, inventoryIds, inventoryNames) {
    if (!purchase) return false;
    const supplierName = normalizeText(purchase.supplier || '');
    const itemId = String(purchase.itemId || '').trim();
    const itemName = normalizeText(purchase.itemName || '');
    const hasSupplier = supplierName && supplierNames.has(supplierName);
    const hasItemById = itemId && inventoryIds.has(itemId);
    const hasItemByName = itemName && inventoryNames.has(itemName);
    return Boolean(hasSupplier || hasItemById || hasItemByName);
}
...
async function loadAuditReport() {
    ...
    try {
        ...
        const [txSnap, purchaseSnap, vehicleSnap, supplierSnap, gstSnap, inventorySnap] = await Promise.all([
        ...
        const tx = txSnap.docs.map((d) => ({ id: d.id, ...d.data() })).filter(isActiveRecord);
        const purchasesAll = purchaseSnap.docs.map((d) => ({ id: d.id, ...d.data() })).filter(isActiveRecord);
        const vehicles = vehicleSnap.docs.map((d) => ({ id: d.id, ...d.data() })).filter(isActiveRecord);
        const suppliers = supplierSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
        const gstDocs = gstSnap.docs.map((d) => ({ id: d.id, ...d.data() })).filter(isActiveRecord);
        const inventory = inventorySnap.docs.map((d) => ({ id: d.id, ...d.data() })).filter(isActiveRecord);

        const supplierIds = new Set(suppliers.map((s) => String(s.id || '').trim()).filter(Boolean));
        const supplierNames = new Set(suppliers.map((s) => normalizeText(s.name || '')).filter(Boolean));
        const inventoryIds = new Set(inventory.map((i) => String(i.id || '').trim()).filter(Boolean));
        const inventoryNames = new Set(inventory.map((i) => normalizeText(i.name || '')).filter(Boolean));

        const invoices = tx.filter((t) => (t.type || '') === 'Invoice');
        const payments = tx.filter((t) => (t.type || '') === 'Payment');
        const supplierPayments = tx
            .filter((t) => (t.type || '') === 'SupplierPayment')
            .filter((p) => isLinkedSupplierPayment(p, supplierIds, supplierNames));
        const purchases = purchasesAll.filter((p) => isLinkedPurchase(p, supplierNames, inventoryIds, inventoryNames));

...
async function deleteProductOption(type, value) {
    ...
    if (!unused) {
        showAlert('danger', `${meta.label} is used in Pipe Category and cannot be deleted`);
        return;
    }
...
function openProductOptionModal(type) {
    ...
    if (help) help.textContent = `Create a new ${meta.label.toLowerCase()} option for Pipe Category.`;
    if (input) {
        input.value = '';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const sidebarProductMasterLink = document.getElementById('sidebarProductMasterLink');
    if (sidebarProductMasterLink) {
        sidebarProductMasterLink.addEventListener('click', (event) => {
            event.preventDefault();
            window.navigateToSection?.('production');
            const modalEl = document.getElementById('productMasterModal');
            if (modalEl) bootstrap.Modal.getOrCreateInstance(modalEl).show();
        });
    }
    const sidebarLocationMasterLink = document.getElementById('sidebarLocationMasterLink');
    if (sidebarLocationMasterLink) {
        sidebarLocationMasterLink.addEventListener('click', (event) => {
            event.preventDefault();
            window.navigateToSection?.('production');
            const modalEl = document.getElementById('locationMasterModal');
            if (modalEl) bootstrap.Modal.getOrCreateInstance(modalEl).show();
        });
    }
...
async function commitInChunks(refsWithData = [], mode = 'set') {
    if (!Array.isArray(refsWithData) || !refsWithData.length) return;
    let batch = db.batch();
    let ops = 0;
    for (const row of refsWithData) {
        if (!row?.ref || !row?.data) continue;
        if (mode === 'update') {
            batch.update(row.ref, row.data);
        } else {
...
async function propagateProductMasterChanges(businessId, productId, oldData = {}, nextData = {}) {
    if (!businessId || !productId) return;
    const userRef = db.collection('users').doc(businessId);
    const oldName = String(oldData?.name || oldData?.productName || '').trim();
    const oldCategory = String(oldData?.category || oldData?.productCategory || '').trim();
    const nextName = String(nextData?.name || '').trim();
    const nextCategory = String(nextData?.category || '').trim();
    const nextUnit = String(nextData?.unit || 'Nos').trim() || 'Nos';
    const nextImage = String(nextData?.imageUrl || '').trim();
    const nextSellingPrice = Number(nextData?.sellingPrice || 0) || 0;
    const nextGstRate = resolveGstRate(nextData?.gstRate);
    const nextLabourRate = Number(nextData?.labourCostPerProduct || 0) || 0;

    const inventoryByMasterSnap = await userRef.collection('inventory').where('productMasterId', '==', productId).get();
    const inventoryByOldNameSnap = oldName
        ? await userRef.collection('inventory').where('name', '==', oldName).get()
        : { docs: [] };

    const inventoryDocs = new Map();
    inventoryByMasterSnap.forEach((doc) => inventoryDocs.set(doc.id, doc));
    inventoryByOldNameSnap.forEach((doc) => {
        const d = doc.data() || {};
        const linkedId = String(d.productMasterId || '').trim();
        const category = String(d.category || d.productType || '').trim();
        if (linkedId && linkedId !== productId) return;
        if (oldCategory && category && category !== oldCategory) return;
        inventoryDocs.set(doc.id, doc);
    });

    const inventoryUpdates = [];
    const touchedInventoryRows = [];
    inventoryDocs.forEach((doc) => {
        const d = doc.data() || {};
        const patch = {
            name: nextName || d.name || 'Product',
            category: nextCategory || d.category || d.productType || 'Finished Goods',
            productType: nextCategory || d.productType || d.category || '',
        ...
        if (nextImage || d.imageUrl) patch.imageUrl = nextImage || d.imageUrl || '';
        inventoryUpdates.push({ ref: doc.ref, data: patch });
        touchedInventoryRows.push({ id: doc.id, ...d, ...patch });
    });
    await commitInChunks(inventoryUpdates, 'set');

    const productionRunsSnap = await userRef.collection('production_runs').where('productMasterId', '==', productId).get();
    const runUpdates = [];
    const payableUpdates = [];
    productionRunsSnap.forEach((doc) => {
        runUpdates.push({
            ref: doc.ref,
            data: {
                finishedGoodName: nextName || (doc.data()?.finishedGoodName || 'Product'),
                productType: nextCategory || (doc.data()?.productType || ''),
                labourRatePerProduct: nextLabourRate,
                updatedAt: new Date()
            }
        });
        const payableRef = userRef.collection('labour_payables').doc(`${doc.id}_labour`);
        payableUpdates.push({
            ref: payableRef,
            data: {
                productName: nextName || 'Product',
                updatedAt: new Date()
            }
        });
    });
    await commitInChunks(runUpdates, 'set');
    await commitInChunks(payableUpdates, 'set');

    if (touchedInventoryRows.length) {
        const publicRef = db.collection('public').doc(businessId);
        const featuredUpdates = [];
        for (const row of touchedInventoryRows) {
            if (!row.showOnLanding) continue;
            const featuredRef = publicRef.collection('featured_stock').doc(row.id);
            featuredUpdates.push({
                ref: featuredRef,
                data: {
                    name: row.name || 'Item',
                    category: row.category || row.productType || 'Other',
                    productCategory: row.category || row.productType || 'Other',
                    productMasterId: productId,
                    type: row.type || '',
        ...
        await commitInChunks(featuredUpdates, 'set');
    }
}
...
async function loadBusinessDefaults() {
    ...
    try {
        ...
        console.warn('Failed to load business defaults for Pipe Category', error);
        appDefaultGstRate = 0;
    }
}
...
async function saveProductMaster() {
    ...
    try {
        ...
        if (currentProductId) {
            const existingDoc = await db.collection('users').doc(businessId).collection('product_master').doc(currentProductId).get();
            oldData = existingDoc.exists ? (existingDoc.data() || {}) : null;
            await db.collection('users').doc(businessId).collection('product_master').doc(currentProductId).update(data);
            await propagateProductMasterChanges(businessId, currentProductId, oldData || {}, data);
            currentProductId = null;
        } else {
...
function syncProductMasterCategoryFields() {
    // Retained for compatibility with existing event bindings.
}
...
window.deleteProductMaster = async (id) => {
    const confirmed = await window.showConfirmAsync('Delete Product', 'Delete this product?', {
        confirmText: 'Delete',
        cancelText: 'Cancel'
    });
    if (!confirmed) return;
...
window.deleteMoldMaster = async (id) => {
    const confirmed = await window.showConfirmAsync('Delete Mold', 'Delete this mold?', {
        confirmText: 'Delete',
        cancelText: 'Cancel'
    });
    if (!confirmed) return;
...
window.deleteLocationMaster = async (id) => {
    const confirmed = await window.showConfirmAsync('Delete Location', 'Delete this location?', {
        confirmText: 'Delete',
        cancelText: 'Cancel'
    });
    if (!confirmed) return;

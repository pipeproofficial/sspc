...
const addRmCategoryBtn = document.getElementById('addRmCategoryBtn');
const addRmUnitBtn = document.getElementById('addRmUnitBtn');
...
async function loadRawMaterials() {
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            if (RAW_CATEGORIES.includes(item.category) || item.materialType === 'raw') {
            ...
            <td class="table-actions-cell text-end">
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                        <li><button class="dropdown-item edit-rm-item" type="button"><i class="fas fa-edit fa-fw me-2"></i>Edit</button></li>
                        <li><button class="dropdown-item view-history-rm" type="button"><i class="fas fa-history fa-fw me-2"></i>History</button></li>
                        <li><button class="dropdown-item stock-in-rm" type="button"><i class="fas fa-arrow-down fa-fw me-2"></i>Stock In</button></li>
                        ${canDelete ? '<li><hr class="dropdown-divider"></li><li><button class="dropdown-item text-danger delete-rm-item" type="button"><i class="fas fa-trash fa-fw me-2"></i>Delete</button></li>' : ''}
                    </ul>
                </div>
            </td>
        </tr>
...
function setupEventListeners() {
    if (addRawMaterialBtn) {
        addRawMaterialBtn.addEventListener('click', (e) => {
            e.preventDefault();
            showAddModal();
        });
    }
    ...
    if (addRmCategoryBtn) {
        addRmCategoryBtn.addEventListener('click', () => addOptionToSelect('rmCategory', 'rmCategoryNew'));
    }
    if (addRmUnitBtn) {
        addRmUnitBtn.addEventListener('click', () => addOptionToSelect('rmUnit', 'rmUnitNew'));
    }
    ...
    const rmCategoryNew = document.getElementById('rmCategoryNew');
    const rmUnitNew = document.getElementById('rmUnitNew');
    if (rmCategoryNew) {
        rmCategoryNew.addEventListener('keydown', (e) => {
            if (e.key !== 'Enter') return;
            e.preventDefault();
            addOptionToSelect('rmCategory', 'rmCategoryNew');
        });
    }
    if (rmUnitNew) {
        rmUnitNew.addEventListener('keydown', (e) => {
            if (e.key !== 'Enter') return;
            e.preventDefault();
            addOptionToSelect('rmUnit', 'rmUnitNew');
        });
    }
...
async function showAddModal() {
    currentItemId = null;
    if (!rawMaterialModal || !rawMaterialForm) return;
    rawMaterialForm.reset();
    document.getElementById('rmModalTitle').textContent = 'Add Raw Material';
    const newCategoryInput = document.getElementById('rmCategoryNew');
    const newUnitInput = document.getElementById('rmUnitNew');
...
async function showEditModal(item) {
    ...
    ensureSelectOption('rmCategory', item.category);
    document.getElementById('rmQuantity').value = item.quantity;
    ensureSelectOption('rmUnit', item.unit);
...
async function viewHistory(item) {
    ...
    try {
        ...
        const [legacySupplySnap, purchaseBillsSnap] = await Promise.all([
        ...
        const rows = [];
        legacySupplySnap.forEach((doc) => {
            const d = doc.data() || {};
            const dateObj = d.date?.toDate ? d.date.toDate() : new Date(d.date || Date.now());
            rows.push({
                dateObj,
                typeLabel: 'Supply In',
                badgeClass: 'bg-success',
                qtyText: `+${Number(d.quantity || 0)}`,
                info: `Supplier: ${d.supplier || 'N/A'}`
            });
        });

        purchaseBillsSnap.forEach((doc) => {
            const bill = doc.data() || {};
            const billDate = bill.date?.toDate ? bill.date.toDate() : new Date(bill.date || Date.now());
            const items = Array.isArray(bill.items) ? bill.items : [];
            items.forEach((line) => {
                if (String(line.materialId || '').trim() !== String(item.id || '').trim()) return;
                ...
                rows.push({
                    ...
                    qtyText: `+${qty}`,
                    info: `Supplier: ${bill.supplierName || 'N/A'} | Ref: ${bill.reference || doc.id}`
                });
            });
        });
        ...
        rows.forEach((row) => {
            const dateTimeStr = row.dateObj.toLocaleString('en-US', {
            ...
            tbody.innerHTML += `
                <tr>
                    ...
                    <td class="fw-bold">${row.qtyText}</td>
...
async function deleteRawMaterial(id) {
    const confirmed = await window.showConfirmAsync('Delete Raw Material', 'Delete this raw material and related supply transactions?', {
        confirmText: 'Delete',
        cancelText: 'Cancel'
    });
    if (!confirmed) return;
    ...
    try {
        const root = db.collection('users').doc(businessId);
        const itemRef = root.collection('inventory').doc(id);
        const itemDoc = await itemRef.get();
        const itemName = String(itemDoc.data()?.name || '').trim();
        const normalizedItemName = normalizeName(itemName);

        const deletedById = await deleteQueryInBatches(root.collection('purchases').where('itemId', '==', id));
        const deletedByName = itemName
            ? await deleteQueryInBatches(root.collection('purchases').where('itemName', '==', itemName))
            : 0;
        let fallbackDeleted = 0;
        if (normalizedItemName) {
            const allPurchasesSnap = await root.collection('purchases').get();
            const docsToDelete = allPurchasesSnap.docs.filter((doc) => {
                const data = doc.data() || {};
                return data.itemId === id || normalizeName(data.itemName || '') === normalizedItemName;
            });
            fallbackDeleted = await deleteDocsInBatches(docsToDelete);
        }

        await itemRef.delete();
        loadRawMaterials();
        window.dispatchEvent(new CustomEvent('paymentsUpdated'));
        const totalDeleted = deletedById + deletedByName + fallbackDeleted;
        if (typeof window.showAlert === 'function') {
            window.showAlert('success', `Raw material deleted${totalDeleted ? ` with ${totalDeleted} related entries removed` : ''}`);
        }

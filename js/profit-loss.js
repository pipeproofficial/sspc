...
function getTransactionDirection(type = '') {
    ...
    if (normalized === 'payment' || normalized === 'payment in' || normalized === 'advance payment' || normalized === 'customer payment') return 'income';
...
function deriveInvoiceCollectedAmount(inv = {}) {
    ...
    if (Number.isFinite(balance) && Number.isFinite(amount)) {
        return Math.max(0, Math.min(amount, amount - balance));
    }
    if (status === 'paid') {
...
function startProfitLossListeners() {
    ...
    // Query all transactions; we filter by date on client so restored string dates also work.
    const txnQuery = basePath.collection('transactions');
    let txnData = [];
    ...
    const txnUnsub = txnQuery.onSnapshot(
        (snap) => {
            ...
            // Capture paid invoice amounts not represented by Payment rows.
            ...
            invoices.forEach(inv => {
                ...
                const expectedCollected = deriveInvoiceCollectedAmount(inv);
                const diff = expectedCollected - recorded;
                if (diff > 0.01) {
                    txnData.push({
                        id: `${inv.id}_initial`,
                        source: 'transaction',
                        type: 'Payment',
                        date: inv.date,
                        description: `Invoice collected amount for ${inv.ref}`,
                        party: inv.party,
                        ref: 'Initial',
                        amount: diff,
                        direction: 'income',

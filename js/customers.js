...
const ADDRESS_LABELS = ['Office', 'Site', 'Plant', 'Warehouse', 'Other'];
...
function createCustomerRow(customer) {
    ...
    const typeBadge = getTypeBadge(customer.type || 'Individual');
    ...
    return `
        <tr data-id="${customer.id}">
            ...
            <td><span class="badge ${typeBadge}">${customer.type || 'Individual'}</span></td>
...
function setupEventListeners() {
    ...
    const addBillBtn = document.getElementById('addBillToAddressBtn');
    if (addBillBtn) addBillBtn.addEventListener('click', () => addAddressRow('bill'));
    const addShipBtn = document.getElementById('addShipToAddressBtn');
    if (addShipBtn) addShipBtn.addEventListener('click', () => addAddressRow('ship'));
    ...
    document.addEventListener('click', (e) => {
        ...
        } else if (e.target.closest('.remove-address-btn')) {
            const card = e.target.closest('.address-row-card');
            const container = card?.parentElement;
            if (card) card.remove();
            if (container && !container.querySelector('.address-row-card')) {
                if (container.id === 'customerBillToContainer') addAddressRow('bill');
                if (container.id === 'customerShipToContainer') addAddressRow('ship');
            } else if (container && !container.querySelector('.address-default:checked')) {
                const first = container.querySelector('.address-default');
                if (first) first.checked = true;
            }
        } else if (e.target.closest('.remove-service-btn')) {
...
function populateCustomerTypeSelect() {
    ...
    typeSelect.innerHTML = CUSTOMER_TYPES.map(type => `<option value="${type}">${type}</option>`).join('');
}
...
function showAddCustomerModal() {
    ...
    const billContainer = document.getElementById('customerBillToContainer');
    const shipContainer = document.getElementById('customerShipToContainer');
    if (billContainer) billContainer.innerHTML = '';
...
function getAddressContainer(kind) {
    return document.getElementById(kind === 'ship' ? 'customerShipToContainer' : 'customerBillToContainer');
}
...
function buildAddressRow(address = {}, kind = 'bill') {
    ...
    const district = (address.district || '').replace(/"/g, '&quot;');
    const mandal = (address.mandal || '').replace(/"/g, '&quot;');
            ...
            <div class="card-body py-2">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control form-control-sm address-line" value="${line}" placeholder="Address line">
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="text" class="form-control form-control-sm address-zip" value="${zip}" placeholder="Pincode">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label small mb-1">Mandal</label>
                        <input type="text" class="form-control form-control-sm address-mandal" value="${mandal}" placeholder="Mandal">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label small mb-1">Village</label>
                        <input type="text" class="form-control form-control-sm address-village" value="${village}" placeholder="Village">
                    </div>
                    <div class="col-md-2">
                        <label class="form-label small mb-1">Default</label>
                        <div class="form-check mt-2">
                            <input class="form-check-input address-default" type="radio" name="${radioName}" ${isDefault ? 'checked' : ''}>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <button type="button" class="btn btn-sm btn-outline-danger w-100 remove-address-btn">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                </div>
...
function addAddressRow(kind = 'bill', address = {}) {
    const container = getAddressContainer(kind);
    if (!container) return;
...
function collectAddresses(kind = 'bill') {
    const container = getAddressContainer(kind);
    if (!container) return [];
    const mapped = Array.from(container.querySelectorAll('.address-row-card')).map(card => ({
        label: card.querySelector('.address-label')?.value?.trim() || '',
        address: card.querySelector('.address-line')?.value?.trim() || '',
        state: card.querySelector('.address-state')?.value?.trim() || '',
        district: card.querySelector('.address-district')?.value?.trim() || '',
        mandal: card.querySelector('.address-mandal')?.value?.trim() || '',
        village: card.querySelector('.address-village')?.value?.trim() || '',
        zip: card.querySelector('.address-zip')?.value?.trim() || '',
        isDefault: Boolean(card.querySelector('.address-default')?.checked)
    })).filter(a => a.address || a.state || a.district || a.mandal || a.village || a.zip || a.label);
    if (mapped.length && !mapped.some(a => a.isDefault)) mapped[0].isDefault = true;
    return mapped;
}
...
async function showEditCustomerModal(customer) {
    ...
    document.getElementById('customerType').value = customer.type || 'Individual';
    ...
    const billToAddressesRaw = Array.isArray(customer.billToAddresses) && customer.billToAddresses.length
        ? customer.billToAddresses
        : [legacyBill];
    const shipToAddressesRaw = Array.isArray(customer.shipToAddresses) && customer.shipToAddresses.length
        ? customer.shipToAddresses
        : (legacyBill.address ? [legacyBill] : []);
    const billToAddresses = billToAddressesRaw.map((a, i) => ({ ...a, isDefault: Boolean(a?.isDefault) || i === 0 }));
    const shipToAddresses = shipToAddressesRaw.map((a, i) => ({ ...a, isDefault: Boolean(a?.isDefault) || i === 0 }));

    billToAddresses.forEach(addr => addAddressRow('bill', addr));
    if (shipToAddresses.length) {
        shipToAddresses.forEach(addr => addAddressRow('ship', addr));
    } else {
        addAddressRow('ship');
    }
...
async function saveCustomer() {
    ...
    // Get form data
    const billToAddresses = collectAddresses('bill');
    const shipToAddresses = collectAddresses('ship');
    const primaryBill = billToAddresses.find(a => a.isDefault) || billToAddresses[0] || {};
    const customerData = {
        ...
        billToAddresses,
        shipToAddresses,
        address: primaryBill.address || '',
        state: primaryBill.state || '',
        zip: primaryBill.zip || '',
...
function validateCustomerForm() {
    ...
    const billToAddresses = collectAddresses('bill');
    if (!billToAddresses.length) {
        showError('Add at least one Bill To address.');
        return false;
    }

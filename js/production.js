...
const productionSummaryBlock = document.getElementById('productionSummaryBlock');
const productionFiltersCard = document.getElementById('productionFiltersCard');
const labourTrackerCard = document.getElementById('labourTrackerCard');
const pipesRunsCard = document.getElementById('pipesRunsCard');
const septicRunsCard = document.getElementById('septicRunsCard');
const curingStatsContainer = document.getElementById('curingStatsContainer');
...
const addProdPipeCategoryBtn = document.getElementById('addProdPipeCategoryBtn');
const productMasterModalEl = document.getElementById('productMasterModal');
...
const labourPaymentsPendingTable = document.getElementById('labourPaymentsPendingTable') || document.getElementById('labourPaymentsTable');
const labourPaymentsCompletedTable = document.getElementById('labourPaymentsCompletedTable');
...
const labourPaymentsTables = [labourPaymentsPendingTable, labourPaymentsCompletedTable].filter(Boolean);
...
let productionViewMode = 'today';
let appDefaultGstRate = 0;
let reopenProductionModalAfterProductMaster = false;
...
async function findFinishedGoodDoc(userRef, pm = {}) {
    ...
    if (pmId) {
        const byMaster = await userRef.collection('inventory').where('productMasterId', '==', pmId).limit(20).get();
        const masterDoc = byMaster.docs.find((doc) => isSameFinishedGoodVariant(doc.data() || {}, pm)) || byMaster.docs[0];
        if (masterDoc) return masterDoc;
    }
...
function toLocationTypeKey(value) {
    return String(value || '').trim().toLowerCase();
}

function findDefaultLocationByType(preferredType = '') {
    const preferred = toLocationTypeKey(preferredType);
    if (!preferred) return null;
    const byType = (locationMasterItems || []).find((loc) => toLocationTypeKey(normalizeLocationType(loc?.type)) === preferred);
    if (byType) return byType;

    const keywords = preferred.includes('production') ? ['production', 'casting']
        : preferred.includes('curing') ? ['curing']
            : preferred.includes('ready') ? ['ready', 'stock']
                : [];
    if (!keywords.length) return null;
    return (locationMasterItems || []).find((loc) => {
        const hay = `${loc?.name || ''} ${loc?.type || ''}`.toLowerCase();
        return keywords.every((k) => hay.includes(k));
    }) || null;
}

function getDefaultLocationId(preferredType = '') {
    const loc = findDefaultLocationByType(preferredType);
    return loc?.id || '';
}
...
window.completeCuring = async (id) => {
    ...
    if (completeReadyLocationSelect) {
        ...
        if (!completeReadyLocationSelect.value) {
            completeReadyLocationSelect.value = getDefaultLocationId('Ready Stock Yard');
        }
    }
...
window.startCuring = async (id) => {
    ...
    if (curingToLocationSelect) {
        ...
        curingToLocationSelect.value = getDefaultLocationId('Curing House');
    }
...
window.allocateSeptic = async (id) => {
    ...
    if (septicAllocationLocationSelect) {
        ...
        if (!septicAllocationLocationSelect.value) {
            septicAllocationLocationSelect.value = getDefaultLocationId('Septic Tank Area');
        }
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    window.addEventListener('sectionChanged', (e) => {
        if (e.detail === 'production') {
            loadProductionHistory();
            applyHashDrivenProductionView();
        }
    });
    window.addEventListener('hashchange', () => {
        if (window.location.hash.startsWith('#production')) {
            applyHashDrivenProductionView();
        }
    });
    ...
    if (addProdPipeCategoryBtn) {
        addProdPipeCategoryBtn.addEventListener('click', () => {
            if (!productMasterModalEl) return;
            reopenProductionModalAfterProductMaster = true;
            bootstrap.Modal.getInstance(productionModal)?.hide();
            bootstrap.Modal.getOrCreateInstance(productMasterModalEl).show();
        });
    }
    if (productMasterModalEl) {
        productMasterModalEl.addEventListener('hidden.bs.modal', async () => {
            if (!reopenProductionModalAfterProductMaster) return;
            reopenProductionModalAfterProductMaster = false;
            const user = JSON.parse(localStorage.getItem('user') || '{}');
            const businessId = user.businessId || user.uid;
            if (businessId) {
                await loadProductionMasters(businessId);
            }
            if (productionModal) bootstrap.Modal.getOrCreateInstance(productionModal).show();
        });
    }
    ...
    if (labourSelectAll) {
        labourSelectAll.addEventListener('change', function () {
            const cbs = labourPaymentsPendingTable ? labourPaymentsPendingTable.querySelectorAll('.labour-row-cb') : [];
    ...
    labourPaymentsTables.forEach((table) => {
    ...
    labourPaymentsTables.forEach((table) => {
        table.addEventListener('click', async (e) => {
            ...
            if (partialBtn) {
                ...
                if (pending <= 0) return;
                const val = await window.showPromptAsync('Record Payment', `Enter payment amount (max Rs ${pending.toFixed(2)})`, pending.toFixed(2), { submitText: 'Save' });
                if (val === null) return;
            ...
            if (reverseBtn) {
                ...
                if (!payableId) return;
                const reason = (await window.showPromptAsync('Reverse Payment', 'Reason for reversal', 'Payment reversal', {
                    submitText: 'Reverse',
                    allowEmpty: true
                })) || 'Payment reversal';
    ...
    if (productionMultiLines) {
        ...
        productionMultiLines.addEventListener('change', (e) => {
            ...
            const row = e.target.closest('.production-line-row');
            if (!row) return;
            if (select) {
                const labourRateInput = row.querySelector('.production-line-labour-rate');
                if (labourRateInput) labourRateInput.value = getProductDefaultLabourRate(select.value);
            }
            renderProductionLineRecipeSummary(row);
        });
        productionMultiLines.addEventListener('input', (e) => {
            const qtyInput = e.target.closest('.production-line-qty');
            if (!qtyInput) return;
            const row = qtyInput.closest('.production-line-row');
            renderProductionLineRecipeSummary(row);
        });
    }
    ...
    if (resetProductionFilters) {
        resetProductionFilters.addEventListener('click', () => {
            ...
            if (productionStageFilter) {
                productionStageFilter.value = (productionViewMode === 'pipes' || productionViewMode === 'septic') ? 'Completed' : 'all';
            }
...
function applyProductionFilters() {
    ...
    const effectiveStageFilter = stageFilter === 'all'
        ? ((productionViewMode === 'pipes' || productionViewMode === 'septic') ? 'Completed'
            : (productionViewMode === 'today' ? '__running__' : 'all'))
        : stageFilter;
    ...
    const filtered = productionDataAll.filter(run => {
        ...
        if (effectiveStageFilter === '__running__' && stage === 'Completed') return false;
        if (effectiveStageFilter !== 'all' && effectiveStageFilter !== '__running__' && stage !== effectiveStageFilter) return false;
        if (fromDate || toDate) {
...
function updateProductionStockCardValue(id, value) {
...
async function updatePipeAndSepticStockCards(businessId) {
    if (!businessId) {
        updateProductionStockCardValue('prodPipesStockAvailable', 0);
        updateProductionStockCardValue('prodSepticStockAvailable', 0);
        return;
    }

    try {
        const inventorySnap = await db.collection('users').doc(businessId).collection('inventory').get();
        let pipesQty = 0;
        let septicQty = 0;
        ...
        inventorySnap.forEach((doc) => {
            ...
            const category = String(item.category || item.productType || '').toLowerCase();
            const name = String(item.name || '').toLowerCase();
            const isSeptic = category.includes('septic') || name.includes('septic');
            const isPipe = category.includes('pipe') || category.includes('rcc') || name.includes('pipe') || name.includes('rcc');
        ...
        updateProductionStockCardValue('prodPipesStockAvailable', pipesQty);
        updateProductionStockCardValue('prodSepticStockAvailable', septicQty);
    } catch (error) {
        console.error('Failed to update production stock cards', error);
        updateProductionStockCardValue('prodPipesStockAvailable', '-');
        updateProductionStockCardValue('prodSepticStockAvailable', '-');
    }
}
...
async function loadProductionHistory() {
    ...
    await updatePipeAndSepticStockCards(businessId);
...
function renderProductionRows(runs, user) {
    ...
    const showCompletedText = productionViewMode === 'pipes' || productionViewMode === 'septic';
    if (!pipeRuns.length) {
        pipesBody.innerHTML = `<tr><td colspan="7" class="text-center text-muted">${showCompletedText ? 'No completed pipe production records found' : 'No running pipe production records found'}</td></tr>`;
    }
    if (!septicRuns.length) {
        septicBody.innerHTML = `<tr><td colspan="7" class="text-center text-muted">${showCompletedText ? 'No completed septic assembly records found' : 'No running septic assembly records found'}</td></tr>`;
    }
    ...
    allRuns.forEach(data => {
        ...
        const sourceText = isSepticAssembly && (data.sourceProductName || data.sourceBatchId)
            ...
            : '';
        const productMetaParts = [data.productType].filter(Boolean);
...
function setProductionView(mode = 'today') {
    const allowedModes = new Set(['today', 'pipes', 'septic', 'labour']);
    productionViewMode = allowedModes.has(mode) ? mode : 'today';

    const isToday = productionViewMode === 'today';
    const isPipes = productionViewMode === 'pipes';
    const isSeptic = productionViewMode === 'septic';
    const isLabour = productionViewMode === 'labour';

    if (productionSummaryBlock) productionSummaryBlock.classList.toggle('d-none', !(isToday));
    if (curingStatsContainer) curingStatsContainer.classList.toggle('d-none', !(isToday));
    if (productionFiltersCard) productionFiltersCard.classList.toggle('d-none', !(isToday || isPipes || isSeptic));
    if (pipesRunsCard) pipesRunsCard.classList.toggle('d-none', !(isToday || isPipes));
    if (septicRunsCard) septicRunsCard.classList.toggle('d-none', !(isToday || isSeptic));
    if (labourTrackerCard) labourTrackerCard.classList.toggle('d-none', !isLabour);

}

function getProductionViewFromHash() {
    const hash = window.location.hash;
    if (hash === '#production-pipes') return 'pipes';
    if (hash === '#production-septic') return 'septic';
    if (hash === '#production-labour-payments') return 'labour';
    return 'today';
}

function applyHashDrivenProductionView() {
    const mode = getProductionViewFromHash();
    setProductionView(mode);
    if (productionStageFilter) {
        productionStageFilter.value = (mode === 'pipes' || mode === 'septic') ? 'Completed' : 'all';
    }
    applyProductionFilters();
}
...
function populateLocationSelects() {
    ...
    if (prodCastingLocationSelect && !prodCastingLocationSelect.value) {
        prodCastingLocationSelect.value = getDefaultLocationId('Production Output');
    }
    if (curingToLocationSelect && !curingToLocationSelect.value) {
        curingToLocationSelect.value = getDefaultLocationId('Curing House');
    }
    if (completeReadyLocationSelect && !completeReadyLocationSelect.value) {
        completeReadyLocationSelect.value = getDefaultLocationId('Ready Stock Yard');
    }
}
...
function populateProductMasterSelect() {
    ...
    if (productMasterItems.length === 0) {
        prodProductMasterSelect.innerHTML = '<option value="">No pipe categories added</option>';
        return;
    }
    prodProductMasterSelect.innerHTML = '<option value="">Select Pipe Category...</option>';
...
function applyProductMasterSelection() {
    ...
    const category = p.category || p.productCategory || '';
    const details = [category].filter(Boolean).join(' | ');
...
function getProductionLineRecipeRows(productMasterId = '', quantity = 0) {
    const pm = (productMasterItems || []).find(p => String(p.id) === String(productMasterId));
    if (!pm) return [];
    const qtyScale = Math.max(0, parseFloat(quantity || '0') || 0);
    const masterIngredients = Array.isArray(pm.rawMaterials) ? pm.rawMaterials : (Array.isArray(pm.ingredients) ? pm.ingredients : []);
    if (!masterIngredients.length || qtyScale <= 0) return [];

    return masterIngredients.map((row) => {
        const id = resolveRecipeMaterialId(row);
        const baseQty = resolveRecipeBaseQty(row);
        if (!id || baseQty <= 0) return null;
        const inv = (inventoryItems || []).find(i => i.id === id);
        return {
            id,
            name: inv?.name || row?.name || row?.materialName || 'Material',
            quantity: roundMoney(baseQty * qtyScale),
            unit: inv?.unit || row?.unit || ''
        };
    }).filter(Boolean);
}

function renderProductionLineRecipeSummary(row) {
    if (!row) return;
    ...
    const summaryEl = row.querySelector('.production-line-rm-summary');
    if (!summaryEl) return;

    if (!productMasterId || quantity <= 0) {
        summaryEl.innerHTML = '<span class="text-muted">Select pipe category and quantity to preview auto raw material deduction.</span>';
        return;
    }

    const recipeRows = getProductionLineRecipeRows(productMasterId, quantity);
    if (!recipeRows.length) {
        summaryEl.innerHTML = '<span class="text-danger">No recipe found for this Pipe Category.</span>';
        return;
    }

    summaryEl.innerHTML = recipeRows.map((r) =>
        `<span class="badge bg-light text-dark border me-1 mb-1">${r.name}: ${r.quantity}${r.unit ? ` ${r.unit}` : ''}</span>`
    ).join('');
}
...
function addProductionLineRow(preSelectedId = '', preQty = '', preLabourRate = '') {
    ...
    const html = `
        <div class="row g-2 mb-2 align-items-end production-line-row" id="${rowId}">
            <div class="col-md-5">
                <label class="form-label small">Pipe Category</label>
            ...
            <div class="col-12">
                <div class="small text-muted mb-1">Raw Materials (Auto)</div>
                <div class="production-line-rm-summary small p-2 bg-light rounded border">
                    <span class="text-muted">Select pipe category and quantity to preview auto raw material deduction.</span>
                </div>
            </div>
        </div>
    `;
    productionMultiLines.insertAdjacentHTML('beforeend', html);
    const row = productionMultiLines.querySelector(`#${rowId}`);
    renderProductionLineRecipeSummary(row);
}
...
function refreshProductionLineOptions() {
    ...
    productionMultiLines.querySelectorAll('.production-line-product').forEach((select) => {
        ...
        renderProductionLineRecipeSummary(select.closest('.production-line-row'));
    });
...
async function saveProductionRun() {
    ...
    if (currentEditId) {
        ...
        if (!selectedProductMaster) return showAlert('danger', 'Please select Pipe Category.');
    ...
    if (missing) return showAlert('danger', 'One or more selected pipe categories are invalid.');

    const lineRunsMissingRecipe = runsToCreate
        .filter(({ line }) => line?.source === 'line')
        .filter(({ pm }) => {
            const recipe = Array.isArray(pm?.rawMaterials) ? pm.rawMaterials : (Array.isArray(pm?.ingredients) ? pm.ingredients : []);
            return recipe.length === 0;
        })
        .map(({ pm }) => (pm?.name || pm?.productName || 'Product'))
        .filter(Boolean);
    if (lineRunsMissingRecipe.length > 0) {
        const list = Array.from(new Set(lineRunsMissingRecipe)).join(', ');
        return showAlert('danger', `Add raw material recipe in Pipe Category for: ${list}. "Add More Pipes" uses auto-detect deduction.`);
    }
    ...
    try {
        ...
        await db.runTransaction(async (tx) => {
            ...
            const preparedRuns = runsToCreate.map(({ line, pm }) => {
                ...
                // If main run has no recipe in Pipe Category, allow manual ingredient rows as fallback.
                let ingredients = [];
...
async function saveSepticAllocation() {
    ...
    try {
        ...
        await db.runTransaction(async (tx) => {
            ...
            const sourceFgId = String(src.finishedGoodId || '').trim();
            if (!sourceFgId) throw new Error('Source finished good is missing.');
            const sourceFgRef = userRef.collection('inventory').doc(sourceFgId);
            const sourceFgDoc = await tx.get(sourceFgRef);
            if (!sourceFgDoc.exists) throw new Error('Source stock item not found.');
            const currentSourceQty = Number(sourceFgDoc.data().quantity || 0);
            if (currentSourceQty < qty) {
                throw new Error(`Insufficient source stock. Available: ${currentSourceQty}`);
            }
            ...
            if (sourceFgId === fgId) {
                tx.update(fgRef, { updatedAt: new Date() });
            } else {
                tx.update(sourceFgRef, {
                    quantity: roundMoney(currentSourceQty - qty),
                    updatedAt: new Date()
                });
...
async function repairLegacySepticStockDeductions() {
    const user = JSON.parse(localStorage.getItem('user') || '{}');
    const businessId = user.businessId || user.uid;
    if (!businessId) return;
    ...
    const confirmed = await window.showConfirmAsync(
        'Repair Septic Stock',
        'This will repair old septic allocations by deducting missing source pipe stock once. Continue?',
        { confirmText: 'Repair', cancelText: 'Cancel' }
    );
    if (!confirmed) return;

    try {
        const userRef = db.collection('users').doc(businessId);
        const runsSnap = await userRef.collection('production_runs').get();
        const runs = runsSnap.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
        const allocRuns = runs
            .filter((run) => String(run.sourceRunId || '').trim() && run.sourceStockDeducted !== true)
            .sort((a, b) => {
                const at = a.date?.toDate ? a.date.toDate().getTime() : new Date(a.date || 0).getTime();
                const bt = b.date?.toDate ? b.date.toDate().getTime() : new Date(b.date || 0).getTime();
                return at - bt;
            });

        if (!allocRuns.length) {
            showAlert('info', 'No legacy septic allocations need repair.');
            return;
        }

        const sourceRunIds = Array.from(new Set(allocRuns.map((run) => String(run.sourceRunId || '').trim()).filter(Boolean)));
        const sourceRunMap = new Map();
        for (const runId of sourceRunIds) {
            // eslint-disable-next-line no-await-in-loop
            const snap = await userRef.collection('production_runs').doc(runId).get();
            if (snap.exists) sourceRunMap.set(runId, { id: snap.id, ...snap.data() });
        }

        const qtyByFgId = new Map();
        let repairedRuns = 0;
        let skippedRuns = 0;
        let sourceUpdates = 0;
        let batch = db.batch();
        ...
        for (const alloc of allocRuns) {
            const allocQty = Number(alloc.quantityProduced || alloc.goodQty || 0);
            if (allocQty <= 0) {
                batch.update(userRef.collection('production_runs').doc(alloc.id), {
                    sourceStockDeducted: true,
                    repairedAt: new Date(),
                    updatedAt: new Date()
                });
            ...
            const sourceRun = sourceRunMap.get(String(alloc.sourceRunId || '').trim());
            const sourceFgId = String(sourceRun?.finishedGoodId || '').trim();
            if (!sourceFgId) {
                skippedRuns += 1;
                continue;
            }

            if (!qtyByFgId.has(sourceFgId)) {
                // eslint-disable-next-line no-await-in-loop
                const fgSnap = await userRef.collection('inventory').doc(sourceFgId).get();
                qtyByFgId.set(sourceFgId, fgSnap.exists ? Number(fgSnap.data().quantity || 0) : 0);
            }

            const currentQty = Number(qtyByFgId.get(sourceFgId) || 0);
            if (currentQty < allocQty) {
                skippedRuns += 1;
                continue;
            }

            const nextQty = roundMoney(currentQty - allocQty);
            qtyByFgId.set(sourceFgId, nextQty);
            batch.update(userRef.collection('production_runs').doc(alloc.id), {
                sourceStockDeducted: true,
                repairedAt: new Date(),
                updatedAt: new Date()
        ...
        for (const [fgId, nextQty] of qtyByFgId.entries()) {
            batch.update(userRef.collection('inventory').doc(fgId), {
                quantity: nextQty,
                updatedAt: new Date()
            });
            ops += 1;
        ...
        await commitBatch();
        showAlert('success', `Repair completed. Fixed ${repairedRuns} allocation run(s), updated ${sourceUpdates} source stock item(s), skipped ${skippedRuns}.`);
        await loadProductionHistory();
    } catch (error) {
        console.error('repairLegacySepticStockDeductions failed', error);
        showAlert('danger', error.message || 'Failed to repair legacy septic allocations.');
    }
}
window.repairLegacySepticStockDeductions = repairLegacySepticStockDeductions;
...
async function deleteProductionRuns(ids = [], options = {}) {
    ...
    if (options.confirm !== false) {
        ...
        const confirmed = await window.showConfirmAsync('Delete Production', `Delete ${label}?`, {
            confirmText: 'Delete',
            cancelText: 'Cancel'
        });
...
async function syncLabourPayablesForRuns(businessId, runs = []) {
    ...
    for (const run of (runs || [])) {
        ...
        const existingStatus = (old?.status || '').toLowerCase();
        const isDeleted = old?.isDeleted === true || existingStatus === 'deleted';
...
function renderLabourPaymentTracker(payables = [], scopedRuns = []) {
    if (!labourPaymentsPendingTable && !labourPaymentsCompletedTable) return;
    const pendingTbody = labourPaymentsPendingTable?.querySelector('tbody') || null;
    const completedTbody = labourPaymentsCompletedTable?.querySelector('tbody') || null;
    ...
    const statusFilterRaw = (labourPayableStatusFilter?.value || 'all').toLowerCase();
    const statusFilter = statusFilterRaw === 'completed' ? 'paid' : statusFilterRaw;
    ...
    const rows = (payables || [])
        ...
        .filter(p => {
            ...
            const text = `${p.productName || ''} ${p.workerName || ''} ${p.producedDate} ${run?.productType || ''}`.toLowerCase();
            if (searchTerm && !text.includes(searchTerm)) return false;
            if (statusFilter === 'pending' && p.status === 'paid') return false;
            if (statusFilter !== 'all' && statusFilter !== 'pending' && p.status !== statusFilter) return false;
    ...
    const pendingRows = rows.filter((p) => p.status !== 'paid');
    const completedRows = rows.filter((p) => p.status === 'paid');

    const rowHtml = (p, selectable = true) => {
            const run = runById.get(p.runId) || {};
            ...
            const selectCell = selectable
                ? `<input type="checkbox" class="form-check-input labour-row-cb" data-id="${p.id}" data-pending="${p.pending}">`
            ...
            return `
                <tr>
                    ...
                    <td class="table-actions-cell text-end">
                        <div class="dropdown">
                            <button class="btn btn-sm btn-outline-secondary table-actions-toggle" type="button" data-bs-toggle="dropdown" data-bs-boundary="window" aria-expanded="false" aria-label="Labour payment actions">
                                <i class="fas fa-ellipsis-v"></i>
                            </button>
                            <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                                <li><button type="button" class="dropdown-item" ${p.runId ? `onclick="window.viewLabourLedger('${p.runId}')"` : 'disabled'}><i class="fas fa-clock-rotate-left fa-fw me-2"></i>View History</button></li>
                                <li><button type="button" class="dropdown-item" ${(p.paid > 0) ? `onclick="window.downloadPaidLabourPdf('${p.id}')"` : 'disabled'}><i class="fas fa-file-pdf fa-fw me-2"></i>Download PDF</button></li>
                                <li><hr class="dropdown-divider"></li>
                                <li><button type="button" class="dropdown-item text-danger delete-labour-payable-btn" data-payable-id="${p.id}"><i class="fas fa-trash fa-fw me-2"></i>Delete</button></li>
                            </ul>
                        </div>
                    </td>
                </tr>`;
    ...
    if (pendingTbody) {
        pendingTbody.innerHTML = pendingRows.length
            ? pendingRows.slice(0, 200).map((p) => rowHtml(p, true)).join('')
            : '<tr><td colspan="10" class="text-center text-muted">No pending labour payables found</td></tr>';
    }
    if (completedTbody) {
        completedTbody.innerHTML = completedRows.length
            ? completedRows.slice(0, 200).map((p) => rowHtml(p, false)).join('')
            : '<tr><td colspan="10" class="text-center text-muted">No completed labour payables found</td></tr>';
    }
...
function updateLabourSelectionButtons() {
    const checked = labourPaymentsTables.flatMap((table) => Array.from(table.querySelectorAll('.labour-row-cb:checked')));
...
function getSelectedLabourIds() {
    const cbs = labourPaymentsTables.flatMap((table) => Array.from(table.querySelectorAll('.labour-row-cb:checked')));
    return cbs.map(cb => cb.dataset.id);
}
...
async function paySelectedLabourPayables() {
    ...
    const confirmed = await window.showConfirmAsync('Confirm Payment', `Pay ${pendingRows.length} selected entries totalling ${formatMoney(totalPending)}?`, {
        confirmText: 'Pay Now',
        cancelText: 'Cancel'
    });
    if (!confirmed) return;
...
async function editLabourPayable(payableId) {
    ...
    try {
        ...
        const val = await window.showPromptAsync('Edit Amount', 'Enter new amount due', String(data.amountDue || 0), { submitText: 'Update' });
...
async function deleteLabourPayable(payableId) {
    ...
    const businessId = user.businessId || user.uid;
    if (!businessId || !payableId) return false;
    const confirmed = await window.showConfirmAsync('Delete Labour Transaction', 'Delete this labour transaction?', {
        confirmText: 'Delete',
        cancelText: 'Cancel'
    });
    if (!confirmed) return false;
...
async function generateLabourInvoicePdf(rows, fromDate, toDate, company = {}) {
    ...
    const itemsRows = sortedRows.map((r, idx) => {
        ...
        const meta = [r.category].filter(Boolean).join(' | ');
...
window.viewLabourLedger = (id, options = {}) => {
    ...
    const productMeta = [run.productType || ''].filter(Boolean).join(' | ');
    const qtyProduced = Number(run.quantityProduced || 0);
    const ratePerProduct = roundMoney(run.labourRatePerProduct || 0);
    const productLabel = `
            ...
            tbody.innerHTML = entries.map(entry => {
                ...
                return `
                    <tr>
                        ...
                        <td class="table-actions-cell text-end">
                            <div class="dropdown">
                                <button class="btn btn-sm btn-outline-secondary table-actions-toggle" type="button" data-bs-toggle="dropdown" data-bs-boundary="window" aria-expanded="false" aria-label="Ledger actions">
                                    <i class="fas fa-ellipsis-v"></i>
                                </button>
                                <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                                    <li><button type="button" class="dropdown-item" ${(entry.paid > 0) ? `onclick="window.downloadPaidLabourPdf('${entry.id}')"` : 'disabled'}><i class="fas fa-file-pdf fa-fw me-2"></i>Download PDF</button></li>
                                    ${canEdit ? `<li><button type="button" class="dropdown-item edit-labour-payable-btn" data-payable-id="${entry.id}"><i class="fas fa-pen fa-fw me-2"></i>Edit</button></li>` : ''}
                                    ${canApprove ? `<li><button type="button" class="dropdown-item approve-labour-payable-btn" data-payable-id="${entry.id}"><i class="fas fa-check fa-fw me-2"></i>Approve</button></li>` : ''}
                                    ${canPay ? `<li><button type="button" class="dropdown-item mark-labour-paid-btn" data-payable-id="${entry.id}"><i class="fas fa-wallet fa-fw me-2"></i>Pay Full</button></li>` : ''}
                                    ${canPay ? `<li><button type="button" class="dropdown-item partial-labour-paid-btn" data-payable-id="${entry.id}" data-pending="${entry.pending}"><i class="fas fa-hand-holding-dollar fa-fw me-2"></i>Partial Pay</button></li>` : ''}
                                    ${canReverse ? `<li><button type="button" class="dropdown-item reverse-labour-payable-btn" data-payable-id="${entry.id}"><i class="fas fa-rotate-left fa-fw me-2"></i>Reverse</button></li>` : ''}
                                    <li><hr class="dropdown-divider"></li>
                                    <li><button type="button" class="dropdown-item text-danger delete-labour-payable-btn" data-payable-id="${entry.id}"><i class="fas fa-trash fa-fw me-2"></i>Delete</button></li>
                                </ul>
                            </div>
                        </td>
                    </tr>

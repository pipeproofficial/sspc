...
async function waitForConfirmDialogCycle(maxWaitMs = 30000) {
    ...
    // Let click handlers open the confirm dialog (if they use one).
    ...
    while (modalEl.classList.contains('show') && (Date.now() - started) < maxWaitMs) {
...
async function deleteSelectedRows(table) {
    ...
    const confirmed = await window.showConfirmAsync('Delete Selected Rows', `Delete ${selectedRows.length} selected row(s)?`, {
        confirmText: 'Delete',
        cancelText: 'Cancel'
    });
    if (!confirmed) return;

    state.deleteBtn.disabled = true;
    const originalLabel = state.deleteBtn.textContent;
    state.deleteBtn.textContent = 'Deleting...';

    try {
        // Inventory rows support true batch deletion to avoid per-row confirm/reload races.
        if (table.id === 'inventoryTable' && typeof window.deleteInventoryItemsBulk === 'function') {
            const itemIds = selectedRows
                .map((row) => String(row.dataset.id || '').trim())
                .filter(Boolean);
            await window.deleteInventoryItemsBulk(itemIds);
            setTimeout(() => syncTableState(table), 400);
            return;
        }

        // Sales docs (invoice/estimate/challan) have cascade side-effects, so delete via dedicated bulk API.
        if ((table.id === 'invoicesTable' || table.id === 'estimatesTable' || table.id === 'challansTable')
            && typeof window.deleteInvoicesBulk === 'function') {
            const docIds = selectedRows
                .map((row) => String(row.dataset.id || '').trim())
                .filter(Boolean);
            await window.deleteInvoicesBulk(docIds);
            setTimeout(() => syncTableState(table), 400);
            return;
        }
        ...
        for (const row of selectedRows) {
            ...
            attempted += 1;
            deleteControl.click();
            // Avoid stacking confirm modals for handlers that prompt before delete.
            // eslint-disable-next-line no-await-in-loop
            await waitForConfirmDialogCycle();
        }
        ...
        state.deleteBtn.textContent = originalLabel || 'Delete Selected';

...
const addExistingStockBtn = document.getElementById('addExistingStockBtn');
const saveExistingStockBtn = document.getElementById('saveExistingStockBtn');
const existingStockModal = document.getElementById('existingStockModal');
const existingStockProductMaster = document.getElementById('existingStockProductMaster');
...
let existingStockProductMasterMap = new Map();
...
function normalizeCategoryLabel(value) {
    ...
    if (normalized === 'rcc pipes') return 'RCC Pipe';
    if (normalized === 'septic tank product' || normalized === 'septic tank products') return 'Septic Tank';
    if (normalized === 'water tank products') return 'Water Tank';
    return raw;
}
...
function buildInventoryMergeKey(item = {}) {
    const n = normalizeText(item.name || '');
    const c = normalizeText(item.category || item.productType || '');
    if (!n || !c) return '';
    return `${n}||${c}`;
}
...
async function loadInventory() {
    ...
    try {
        ...
        inventorySnapshot.forEach(doc => {
            ...
            const mergeKey = buildInventoryMergeKey(item) || `id:${item.id}`;
            const existing = groupedInventory.get(mergeKey);
            if (!existing) {
                groupedInventory.set(mergeKey, {
                    ...item,
                    quantity: Number(item.quantity || 0),
                    mergedItemIds: [item.id],
                    mergedSources: new Set([String(item.source || '').trim().toLowerCase()])
                });
                return;
            }
            ...
            existing.mergedSources.add(String(item.source || '').trim().toLowerCase());
            if (!existing.productMasterId && item.productMasterId) existing.productMasterId = item.productMasterId;
            if (!existing.hsn && item.hsn) existing.hsn = item.hsn;
            if (!existing.pipeType && item.pipeType) existing.pipeType = item.pipeType;
            if (!existing.loadClass && item.loadClass) existing.loadClass = item.loadClass;
            if (!existing.imageUrl && item.imageUrl) existing.imageUrl = item.imageUrl;
            if (!Number(existing.sellingPrice || 0) && Number(item.sellingPrice || 0)) existing.sellingPrice = Number(item.sellingPrice || 0);
            if (!Number(existing.costPrice || 0) && Number(item.costPrice || 0)) existing.costPrice = Number(item.costPrice || 0);
            if (!Number(existing.reorderLevel || 0) && Number(item.reorderLevel || 0)) existing.reorderLevel = Number(item.reorderLevel || 0);
        });

        inventoryData = Array.from(groupedInventory.values()).map((item) => {
            const sources = Array.from(item.mergedSources || []).filter(Boolean);
            if (sources.length > 1) item.source = 'mixed';
            delete item.mergedSources;
            return item;
        });
        inventoryData.sort((a, b) => String(a.name || '').localeCompare(String(b.name || '')));

        const rowsHtml = inventoryData.map(createInventoryRow).join('');
...
function createInventoryRow(item) {
    ...
    if (String(item.source || '').toLowerCase() === 'opening_stock') {
        metaHtml += `<div class="mt-1"><span class="badge bg-info-subtle text-info border">Opening Stock</span></div>`;
    }
            ...
            <td class="table-actions-cell text-end">
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                        <li>
                            <button class="dropdown-item view-history" type="button">
                                <i class="fas fa-history fa-fw me-2"></i>View History
                            </button>
                        </li>
                        <li>
                            <button class="dropdown-item toggle-landing" type="button">
                                <i class="fas fa-star fa-fw me-2 ${showOnLanding ? 'text-warning' : 'text-muted'}"></i>${showOnLanding ? 'Remove from Landing' : 'Show on Landing'}
                            </button>
                        </li>
                        ${canDelete ? `<li><hr class="dropdown-divider"></li>
                        <li>
                            <button class="dropdown-item text-danger delete-item" type="button">
                                <i class="fas fa-trash fa-fw me-2"></i>Delete Item
                            </button>
                        </li>` : ''}
                    </ul>
                </div>
...
function setupEventListeners() {
    // Add Item Button
    if (addExistingStockBtn) {
        addExistingStockBtn.addEventListener('click', openExistingStockModal);
    }
    if (saveExistingStockBtn) {
        saveExistingStockBtn.addEventListener('click', saveExistingStockItem);
    }
    if (existingStockProductMaster) {
        existingStockProductMaster.addEventListener('change', () => {
            if (existingStockProductMaster.value === '__add_pipe_category__') {
                existingStockProductMaster.value = '';
                bootstrap.Modal.getOrCreateInstance(existingStockModal)?.hide();
                const pmModalEl = document.getElementById('productMasterModal');
                if (pmModalEl) {
                    bootstrap.Modal.getOrCreateInstance(pmModalEl).show();
                } else {
                    showError('Pipe Category screen not available.');
                }
                return;
            }
            applyExistingStockProductMaster(existingStockProductMaster.value || '');
        });
    }
...
function exportInventoryCSV() {
    ...
    const headers = ['Item', 'Category', 'HSN/SAC', 'GST%', 'Qty', 'Unit', 'Price'];
    ...
    try {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            transaction.set(purchaseRef, {
                ...
                receivedBy,
                remarks,
                source: 'stock_adjustment_in'
            });
        });

        // Supplier payable is posted via Purchase Bills; stock-in is inventory-only.
...
async function deleteInventoryItem(itemId) {
    window.showConfirm('Delete Item', 'Are you Sure you want to delete this item?', async () => {
        ...
        try {
            const selected = inventoryData.find((row) => String(row.id) === String(itemId));
            const targetIds = Array.from(new Set(
                (selected?.mergedItemIds?.length ? selected.mergedItemIds : [itemId])
...
async function deleteInventoryItemsBulk(itemIds = []) {
    const expandedIds = [];
    (itemIds || []).forEach((id) => {
        const row = inventoryData.find((r) => String(r.id) === String(id));
        if (row?.mergedItemIds?.length) {
            expandedIds.push(...row.mergedItemIds);
        } else {
            expandedIds.push(id);
        }
    });
    const ids = Array.from(new Set(expandedIds.map((id) => String(id || '').trim()).filter(Boolean)));
    ...
    try {
        ...
        for (const itemId of ids) {
            ...
            ops += 2;
            if (ops >= 400) {
                // Commit in chunks to stay under Firestore write limits.
                // eslint-disable-next-line no-await-in-loop
                await commitBatch();
            }
        }
        ...
        await commitBatch();
        showSuccess(`Deleted ${ids.length} inventory item(s).`);
        await loadInventory();
    } catch (error) {
        console.error('Error deleting inventory items in bulk:', error);
        showError('Failed to delete selected inventory items.');
    }
}

window.deleteInventoryItemsBulk = deleteInventoryItemsBulk;
...
function applyInventoryFilters() {
    ...
    if (!filtered.length) {
        tbody.innerHTML = `
            <tr>
                <td colspan="8" class="text-center py-4 text-muted">No items match the filters</td>
            </tr>
        `;
    } else {
...
async function loadExistingStockProductMasterOptions() {
    if (!existingStockProductMaster) return;
    const user = JSON.parse(localStorage.getItem('user') || '{}');
    const businessId = user.businessId || user.uid;
    if (!businessId) return;

    existingStockProductMaster.innerHTML = '<option value="">Loading...</option>';
    existingStockProductMasterMap = new Map();

    try {
        const snap = await db.collection('users').doc(businessId).collection('product_master').orderBy('name').get();
        existingStockProductMaster.innerHTML = '<option value="">Manual Entry</option>';
        snap.forEach((doc) => {
            const p = doc.data() || {};
            existingStockProductMasterMap.set(doc.id, { id: doc.id, ...p });
            const name = String(p.name || p.productName || '').trim();
            const category = normalizeCategoryLabel(String(p.category || p.productCategory || '').trim());
            const option = document.createElement('option');
            option.value = doc.id;
            option.textContent = category ? `${name} | ${category}` : name || doc.id;
            existingStockProductMaster.appendChild(option);
        });
        existingStockProductMaster.innerHTML += '<option value="__add_pipe_category__">+ Add Pipe Category...</option>';
    } catch (error) {
        console.error('Failed to load product master for existing stock', error);
        existingStockProductMaster.innerHTML = '<option value="">Manual Entry</option>';
    }
}

function applyExistingStockProductMaster(productMasterId = '') {
    const selected = existingStockProductMasterMap.get(String(productMasterId || '').trim());
    if (!selected) return;
    ...
    setVal('existingStockName', selected.name || selected.productName || '');
    setVal('existingStockCategory', normalizeCategoryLabel(selected.category || selected.productCategory || ''));
    setVal('existingStockUnit', selected.unit || 'Nos');
    setVal('existingStockPrice', Number(selected.sellingPrice || 0));
}

async function openExistingStockModal() {
    if (!existingStockModal) return;
    const defaults = {
        existingStockProductMaster: '',
        existingStockName: '',
        existingStockCategory: '',
        existingStockQty: '',
        existingStockUnit: 'Nos',
        existingStockPrice: '0'
    };
    Object.entries(defaults).forEach(([id, value]) => {
        const el = document.getElementById(id);
        if (el) el.value = value;
    });
    await loadExistingStockProductMasterOptions();
    bootstrap.Modal.getOrCreateInstance(existingStockModal).show();
}

async function openExistingStockModalWithPrefill(prefill = {}) {
    if (!existingStockModal) return;
    await openExistingStockModal();
    const setVal = (id, value) => {
        const el = document.getElementById(id);
        if (!el) return;
        el.value = value ?? '';
    };
    const productMasterId = String(prefill.productMasterId || '').trim();
    if (productMasterId && existingStockProductMaster) {
        existingStockProductMaster.value = productMasterId;
        applyExistingStockProductMaster(productMasterId);
    }
    if (prefill.name) setVal('existingStockName', String(prefill.name).trim());
    if (prefill.category) setVal('existingStockCategory', String(prefill.category).trim());
    if (prefill.unit) setVal('existingStockUnit', String(prefill.unit).trim());
    if (prefill.sellingPrice !== undefined) setVal('existingStockPrice', Number(prefill.sellingPrice || 0));
}

async function saveExistingStockItem() {
    const user = JSON.parse(localStorage.getItem('user') || '{}');
    const businessId = user.businessId || user.uid;
    if (!businessId) return;

    const selectedProductMasterId = String(document.getElementById('existingStockProductMaster')?.value || '').trim();
    const name = (document.getElementById('existingStockName')?.value || '').trim();
    const category = normalizeCategoryLabel((document.getElementById('existingStockCategory')?.value || '').trim());
    const qty = Number(document.getElementById('existingStockQty')?.value || 0);
    const unit = (document.getElementById('existingStockUnit')?.value || 'Nos').trim() || 'Nos';
    const sellingPriceInput = Number(document.getElementById('existingStockPrice')?.value || 0);

    if (!name) return showError('Item name is required.');
    if (!category) return showError('Category is required.');
    if (!Number.isFinite(qty) || qty <= 0) return showError('Opening quantity should be greater than 0.');

    const normalizedName = normalizeText(name);
    const normalizedCategory = normalizeText(category);
    const selectedMaster = selectedProductMasterId ? (existingStockProductMasterMap.get(selectedProductMasterId) || null) : null;
    const matchedMaster = selectedMaster || Array.from(existingStockProductMasterMap.values()).find((pm) => {
        const pmName = normalizeText(pm?.name || pm?.productName || '');
        const pmCategory = normalizeText(pm?.category || pm?.productCategory || '');
        return pmName === normalizedName && pmCategory === normalizedCategory;
    }) || null;
    const resolvedProductMasterId = selectedProductMasterId || String(matchedMaster?.id || '').trim();
    const costPrice = Number(matchedMaster?.costPrice || 0) || 0;
    const sellingPrice = Number.isFinite(sellingPriceInput) && sellingPriceInput > 0
        ? sellingPriceInput
        : (Number(matchedMaster?.sellingPrice || 0) || 0);
    const reorderLevel = Math.max(0, parseInt(matchedMaster?.reorderLevel || '0', 10) || 0);
    const hsn = String(matchedMaster?.hsn || '').trim();
    const gstRate = resolveGstRate(matchedMaster?.gstRate || 0);

    try {
        let exactDoc = null;
        if (resolvedProductMasterId) {
            const byMaster = await db.collection('users').doc(businessId)
                .collection('inventory')
                .where('productMasterId', '==', resolvedProductMasterId)
                .limit(20)
                .get();
            if (!byMaster.empty) {
                exactDoc = byMaster.docs.find((doc) => normalizeText(doc.data()?.category || doc.data()?.productType || '') === normalizedCategory)
                    || byMaster.docs[0];
            }
        } else {
            const existingSnap = await db.collection('users').doc(businessId)
                .collection('inventory')
                .where('name', '==', name)
                .limit(20)
                .get();
            exactDoc = existingSnap.docs.find((doc) => normalizeText(doc.data()?.category || doc.data()?.productType || '') === normalizedCategory) || null;
        }

        if (exactDoc) {
            const data = exactDoc.data() || {};
            await exactDoc.ref.update({
                ...
                productMasterId: resolvedProductMasterId || data.productMasterId || '',
                updatedAt: new Date()
            });
        } else {
            await db.collection('users').doc(businessId).collection('inventory').add({
                name,
                sku: '',
                category,
                productType: category,
                productMasterId: resolvedProductMasterId || null,
                type: 'Opening',
                description: '',
                quantity: qty,
                unit,
        ...
        bootstrap.Modal.getOrCreateInstance(existingStockModal).hide();
        showSuccess('Existing stock saved successfully.');
        await loadInventory();
    } catch (error) {
        console.error('saveExistingStockItem failed', error);
        showError('Failed to save existing stock.');
    }
}

window.openExistingStockModal = () => { openExistingStockModal(); };
window.openExistingStockModalWithProduct = (product = {}) => { openExistingStockModalWithPrefill(product); };
